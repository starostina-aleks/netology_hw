...
class SupportAssistantApp:
    def __init__(self, settings: Settings) -> None:
        ...
        self.tracker = BudgetTracker(daily_budget=20, alert_threshold=0.8)
    ...
    def respond(self, user_message: str, image_path: str | None = None) -> AssistantResponse:
        ...
        if not image_path:       
            ...
            if cached is not None:
                ...
                self._log(user_message, cached, 0, latency, True, "cache", "cache")
                return AssistantResponse(cached, True, latency, "cache", "cache")
    ...
    def _log(
        ...
        logger.info(
            "{prov}/{mod} | {tok} tok | {lat:.3f}s | cache={cache} | Q: {msg} | A: {ans}",
            prov=provider,
            mod=model,
            tok=tokens,
            lat=latency_seconds,
